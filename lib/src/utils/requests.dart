...
int _parseExifDimension(Map<String, IfdTag> exifData, bool isHeight) {
  final raw = isHeight
...
double? _parseExifDouble(String? raw) {
...
bool _isScreenshot(Map<String, IfdTag> exifData, int width, int height) {
...
Future<void> _processEntry(dynamic entry) async {
  ...
  if (mimetype.contains('image/')) {
    try {
      if (detectBackend() == ServerBackend.copyparty) {
        imageBytes = await CopypartyService.fetchFileRange(path);
      } else {
        final isHeic = mimetype == 'image/heic';
  ...
  int width = _parseExifDimension(exifData, false);
  int height = _parseExifDimension(exifData, true);
    ...
    try {
      if (detectBackend() == ServerBackend.copyparty) {
        controller = VideoPlayerController.networkUrl(
          Uri.parse("${CopypartyService.baseUrl}/photos/$path"),
          httpHeaders: {
        ...
        controller = VideoPlayerController.networkUrl(
          Uri.parse(
              "https://${box.get('apiDomain')}:${box.get('httpsPort')}/api/v15/dl/$path"),
      ...
      log('Video init failed for $path: $e');
    } finally {
  ...
  await PhotoStore.addPhoto(
    ...
    focalLength: _parseExifDouble(
            exifData['EXIF FocalLengthIn35mmFilm']?.printable ??
                exifData['EXIF FocalLength']?.printable)
        ?.round(),
    exposureValue:
        _parseExifDouble(exifData['EXIF ExposureBiasValue']?.printable)?.round(),
    focus: _parseExifDouble(
            exifData['EXIF FNumber']?.printable.replaceAll('ƒ/', ''))
        ?.round(),
    isScreenshot: _isScreenshot(exifData, width, height),
  );
}
...
Future<List<dynamic>> fetchPhotosDir() async {
  ...
  switch (detectBackend()) {
    ...
    case ServerBackend.freebox:
      final directories =
          await client?.fetch(url: "v15/fs/ls/L0ZyZWVib3gvVGVzdA==");
  ...
  final newFiles =
      all.where((e) => PhotoStore.get(e['path']) == null).toList();
  log('[scan] ${all.length} fichiers, ${newFiles.length} nouveaux');

  const concurrency = 4;
  for (var i = 0; i < newFiles.length; i += concurrency) {
    await Future.wait(newFiles.skip(i).take(concurrency).map(_processEntry));
  }
